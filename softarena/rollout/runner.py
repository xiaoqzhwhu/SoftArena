...
def scripted_archive_forensics_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    steps = []
    extract_obs = runtime.call("bin2mcp/busybox-mcp/decompress_file", {"archive_path": str(archive_path), "output_dir": str(extract_dir)})
    members = extract_obs.get("content", {}).get("members", [])
    steps.append(make_step(0, "bin2mcp/busybox-mcp/decompress_file", {"archive_path": str(archive_path), "output_dir": str(extract_dir)}, "Extract the archive so files can be inspected.", extract_obs))
    evidence = extract_dir / members[0]
    file_obs = runtime.call("bin2mcp/file-mcp/identify_file", {"path": str(evidence)})
    file_type = file_obs.get("content", {}).get("file_type", "unknown")
    hash_obs = runtime.call("bin2mcp/xxhash-mcp/xxhash_compute", {"data": evidence.read_text(), "algorithm": "sha256"})
    digest = hash_obs.get("content", {}).get("hash", "")
    steps.append(make_step(1, "bin2mcp/file-mcp/identify_file", {"path": str(evidence)}, "Identify the extracted evidence file type.", file_obs))
    steps.append(make_step(2, "bin2mcp/xxhash-mcp/xxhash_compute", {"data": evidence.read_text(), "algorithm": "sha256"}, "Hash the evidence to make the report reproducible.", hash_obs))
    Path(episode["report_path"]).write_text(json.dumps({"evidence_file": evidence.name, "sha256": digest, "file_type": file_type}, indent=2) + "\n")
    steps.append(make_step(3, "bin2mcp/busybox-mcp/write_file", {"path": episode["report_path"]}, "Write the final forensic report.", {"ok": True}))
    return steps
...
def scripted_text_transform_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    return [
        make_step(0, "bin2mcp/busybox-mcp/read_file", {"path": str(input_path)}, "Read the CSV rows before filtering.", {"ok": True, "rows": len(rows)}),
        make_step(1, "bin2mcp/ffjson-mcp/generate_ffjson", {"filter": "status == active; email lowercase"}, "Filter active contacts and normalize email addresses.", {"ok": True, "rows": len(active)}),
        make_step(2, "bin2mcp/busybox-mcp/write_file", {"path": str(output_path)}, "Write the semantic JSONL output.", {"ok": True}),
    ]
...
def scripted_build_fix_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    before_obs = runtime.call("bin2mcp/bmake-mcp/run_build", {"cwd": str(project), "target": "test"})
    source.write_text(source.read_text().replace("return a - b;", "return a + b;"))
    after_obs = runtime.call("bin2mcp/bmake-mcp/run_build", {"cwd": str(project), "target": "test"})
    return [
        make_step(0, "bin2mcp/bmake-mcp/run_build", {"cwd": str(project)}, "Run the hidden-equivalent build test to reproduce the failure.", before_obs),
        make_step(1, "bin2mcp/busybox-mcp/write_file", {"path": str(source)}, "Fix the arithmetic bug in the source file.", {"ok": True}),
        make_step(2, "bin2mcp/bmake-mcp/run_build", {"cwd": str(project)}, "Run tests again after the source fix.", after_obs),
    ]
...
def scripted_dns_debug_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    return [
        make_step(0, "bin2mcp/bind9-dnsutils-mcp/dns_lookup", {"domain": evidence["domain"]}, "Check the mocked DNS answer.", {"ok": True, "answer": evidence["dns_answer"]}),
        make_step(1, "bin2mcp/bind9-dnsutils-mcp/dns_ping", {"domain": evidence["domain"]}, "Compare the service endpoint observed by HTTP evidence.", {"ok": True, "connect_ip": evidence["http_connect_ip"]}),
        make_step(2, "bin2mcp/busybox-mcp/write_file", {"path": episode["report_path"]}, "Report the stale DNS record and remediation.", {"ok": True}),
    ]
...
def scripted_accounting_reconcile_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    """
    obs = runtime.call("bin2mcp/postgresql-client-mcp/sql_execute", {"db_path": episode["db_path"], "sql": sql})
    query = runtime.call("bin2mcp/postgresql-client-mcp/sql_execute", {"db_path": episode["db_path"], "sql": "SELECT * FROM reconciliation_report ORDER BY account_id"})
    return [
...
def scripted_sqlite_policy(episode: dict[str, Any], runtime: ToolRuntime) -> list[dict[str, Any]]:
    ...
    actions = [
        {
            "name": "bin2mcp/postgresql-client-mcp/sql_describe",
            "arguments": {"db_path": db_path},
            "rationale": "Inspect the existing database schema before changing state.",
        },
        {
            "name": "bin2mcp/postgresql-client-mcp/sql_execute",
            "arguments": {
                "db_path": db_path,
                "sql": (
